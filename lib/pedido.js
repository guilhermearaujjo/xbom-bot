...
const { BACKEND_URL, ORIGEM, PROMO_FRETE } = require("./config");

const MARCACAO_PROMO = PROMO_FRETE.marcacaoCupom;
...
function montarCorpo({ pedido, pagamento }) {
  ...
  // Este arquivo cuida SÓ de pagamento na entrega/balcão (dinheiro e cartão).
  // Pix tem caminho próprio em lib/pix.js, porque o create-pix do backend já
  // cria o pedido por dentro — passar por aqui geraria pedido duplicado.
  const paymentType = "PAGAR_DEPOIS";
  const metodoNaEntrega = tipo === "cartao" ? "cartao" : "dinheiro";

  const nome = MODO_TESTE ? `TESTE — ${pedido.nome}` : pedido.nome;

  // Cozinha e entregador precisam ver por que a entrega saiu de graça
  const marcaPromo = pedido.promoFrete ? `[${MARCACAO_PROMO}] ` : "";
  ...
  return {
    ...
    obs: `${marcaPromo}${pedido.observacoes || ""}`.trim(),
    origem: ORIGEM
  };
}
