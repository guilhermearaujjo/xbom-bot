...
// -----------------------------------------------------------------------------
// Busca a rua e o bairro pelo CEP (ViaCEP, gratuito e sem chave).
//
// Serve só para a Ana CONFIRMAR o endereço com o cliente ("Rua tal, bairro tal,
// certo? Qual o número?") em vez de fazer a pessoa digitar tudo. Quem decide se
// entrega ou não continua sendo a regra de prefixo + raio acima.
//
// Devolve null se o CEP não existir ou a consulta falhar — nesse caso a Ana
// simplesmente pede a rua, como antes.
// -----------------------------------------------------------------------------
async function buscarEnderecoPorCep(cep) {
  const limpo = normalizarCep(cep);
  if (!formatoCepValido(limpo)) return null;

  const controle = new AbortController();
  const alarme = setTimeout(() => controle.abort(), 5000);
  ...
  try {
    ...
    return {
      ...
      cidade: d.localidade || "",
      uf: d.uf || "",
      atendido: cepNaAreaAtendida(limpo)
    };
  } catch (err) {
    clearTimeout(alarme);
    console.error("[entrega] ViaCEP falhou:", err.name === "AbortError" ? "timeout" : err.message);
    return null;
  }
}

module.exports.buscarEnderecoPorCep = buscarEnderecoPorCep;
