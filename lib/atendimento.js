...
async function processarMensagem({
  ...
  try {
    ...
    if (conversa.aguardandoConfirmacao && confirmou(texto)) {
      ...
      const msgs = ehPix
        ? [
            "Pedido registrado! Segue o Pix — é só copiar o código abaixo e pagar pelo app do seu banco. Assim que cair, a cozinha começa a preparar 🙂",
            criacao.copiaECola
          ]
        : [
            `Pedido confirmado! Já mandei pra cozinha. ${
              validacao.pedido.formaEntrega === "entrega"
                ? "Logo mais o entregador sai com ele."
                : "É só vir buscar no balcão quando estiver pronto."
            }`
          ];
