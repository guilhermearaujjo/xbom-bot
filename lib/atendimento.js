...
async function processarMensagem({
  ...
  try {
    ...
    if (conversa.aguardandoConfirmacao && confirmou(texto)) {
      ...
      const criacao = await criarPedido({
      ...
      const ehPix = String(conversa.pagamento?.tipo || "").toLowerCase() === "pix";

      const msgs = ehPix
        ? [
            `Pedido registrado! Vou te mandar o Pix agora pra você pagar. Assim que cair, a cozinha começa a preparar.`
          ]
        : [
