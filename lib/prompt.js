...
function cardapioEmTexto(cardapio) {
  const nomeCategoria = Object.fromEntries(
    (cardapio.categorias || []).map((c) => [c.id, c.nome])
  );
...
- "pedirConfirmacao": true SOMENTE quando tiver itens, forma de entrega, endereço
  (se for entrega), nome e forma de pagamento — e você acabou de ler o resumo do
  pedido em voz alta pedindo o "confirma?".
